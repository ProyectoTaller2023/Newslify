...
namespace Newslify
{
    public class NewsAppService: NewslifyAppService, INewsAppService
    {
        ...
        public async Task<ICollection<NewsDto>> GetNewsAsync(string LanguageIntCode, int? amountNews, string? query)
         {
            // Aca estaria el monitoreo de tiempo de respuesta de la API para independizar
            //TODO: Control de acceso tiempo a la API
           ICollection <ArticleDto> articles = await _newsService.getNews(LanguageIntCode.ToUpper(),amountNews, query);// metodo que se conecte a la API y traiga las noticias
